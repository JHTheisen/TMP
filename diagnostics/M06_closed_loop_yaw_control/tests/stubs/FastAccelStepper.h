...
public:
    ...
    int8_t setSpeedInHz(uint32_t value) { speed = value; return simulated::speedFails ? -1 : 0; }
    int8_t setAcceleration(int32_t value) { acceleration = value; return simulated::accelerationFails ? -1 : 0; }
    int8_t move(int32_t steps)
    {
        if (simulated::moveRejected) return -1;
        simulated::startSteps = simulated::position();
        const double yawAt = simulated::actualYaw();
        ...
        simulated::commands.push_back({stepPin, steps, speed, acceleration, millis(), yawAt});
        return MOVE_OK;
    }
