...
int main(int argc, char **argv) {
    ...
    const bool successful = oneOf(scenario, {"normal", "wrap_positive", "wrap_negative",
        "brief_gap", "fresh_after_stop", "startup_reset", "blocked_uart", "low_gain", "first_test"});
    const bool pitchReadyOnly = oneOf(scenario, {"baseline_low", "baseline_intermittent"});
    ...
    while (!finalPrinted && millis() < 120000) {
        ...
        if (!injected && (seedNow || movingNow || brakeNow || burstNow || stoppedNow || slewCommandNow)) {
            ...
            if (scenario == "settle_running") {
                ...
                heading.continuous = northTargetContinuous; orientation.roll = 0;
            ...
            if (scenario == "pitch_guard_positive") orientation.roll = 75.1;
            if (scenario == "pitch_guard_negative") orientation.roll = -75.1;
    ...
    CHECK(finalPrinted);
    // Terminal output is queued after stop; an unavailable UART must not stall
    // BNO acquisition while successful startup is waiting for pose commands.
    if (scenario == "blocked_uart") simulated::serialWriteSpace = 128;
    while (txOffset < txLength && millis() - finishedAt < 5000) loop();
    CHECK(txOffset == txLength);
    CHECK(occurrences("FINAL RESULT: ") == (pitchReadyOnly ? 0U : 1U));
    ...
    } else if (pitchReadyOnly) {
        CHECK(phase == Phase::COMPLETE && pitchReady && !referenceSet && !northUsable);
        CHECK(simulated::commands.empty() && printed("PITCH-ONLY READY"));
    } else {
        CHECK(phase == Phase::ABORTED && printed("FINAL RESULT: FAIL"));
        if (oneOf(scenario, {"accuracy_brief", "accuracy_repeated", "accuracy_sustained"})) {
            CHECK(injected && sawAccuracyLow && !survivedAccuracyLow && printed("accuracy below 2: yaw operation stopped"));
            CHECK(finishedAt - injectedAt < 50);
            CHECK(simulated::commands.size() == commandsAtInjection);
            CHECK(!motionWatchdog.tripped());
        }
    ...
    if (successful || pitchReadyOnly) {
        CHECK(simulated::bnoReads > savedReads); // Pose-command idle keeps orientation current.
        CHECK(!simulated::motors[2].position);
    } else {
        CHECK(Serial.output == savedOutput && simulated::bnoReads == savedReads);
    }
    CHECK(!yawMotor || !yawMotor->isRunning());
    CHECK(!pitchMotor || !pitchMotor->isRunning());
