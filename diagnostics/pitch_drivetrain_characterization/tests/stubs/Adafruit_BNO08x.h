...
public:
    ...
    bool getSensorEvent(sh2_SensorValue_t *event)
    {
        ...
        event->status = 3;
        double pitchDegrees = 5.0 + simulated::pitchSign * simulated::position() * 360.0 / 3200.0 / simulated::fixtureRatio;
        if (simulated::noisyBno) pitchDegrees += 0.5 * sin(millis() * 0.1);
        const double halfAngle = pitchDegrees / RAD_TO_DEG / 2.0;
        event->un.rotationVector = {static_cast<float>(cos(halfAngle)), 0.0F, static_cast<float>(sin(halfAngle)), 0.0F};
        return true;
    }
protected:
