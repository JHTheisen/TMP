...
public:
    ...
    bool getSensorEvent(sh2_SensorValue_t *event)
    {
        ...
        const double pitchDegrees = simulated::actualPitch() +
                                    simulated::noiseAmplitude * sin(millis() * 0.027);
        const double halfAngle = pitchDegrees / RAD_TO_DEG / 2.0;
        event->un.rotationVector = {static_cast<float>(cos(halfAngle)), 0.0F,
                                   static_cast<float>(sin(halfAngle)), 0.0F};
        return true;
    }
protected:
...
private:
    ...
    { return simulated::reportInitFails ? 0 : static_cast<int>(length); }
    bool enabled_ = false;
